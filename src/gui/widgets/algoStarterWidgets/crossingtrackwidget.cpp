    ...
    m_progress( 0 )
{
    ...
    for ( int k = 0; k < dsl.size(); ++k )
    {
        ...
        {
            m_maskSelect->addItem( VPtr<Dataset>::asPtr( dsl[k] )->properties()->get( Fn::Property::NAME ).toString(), k+1 );
        }
    }
    ...
    for ( int k = 0; k < dsl.size(); ++k )
    {
        if ( VPtr<Dataset>::asPtr( dsl[k] )->properties()->get( Fn::Property::TYPE ).toInt() == (int)Fn::DatasetType::NIFTI_TENSOR )
        {
            m_tensor2Select->addItem( VPtr<Dataset>::asPtr( dsl[k] )->properties()->get( Fn::Property::NAME ).toString(), k+1 );
        }
    }
    ...
    for ( int k = 0; k < dsl.size(); ++k )
    {
        if ( VPtr<Dataset>::asPtr( dsl[k] )->properties()->get( Fn::Property::TYPE ).toInt() == (int)Fn::DatasetType::NIFTI_TENSOR )
        {
            m_tensor3Select->addItem( VPtr<Dataset>::asPtr( dsl[k] )->properties()->get( Fn::Property::NAME ).toString(), k+1 );
        }
    }
...
QList<Dataset*> CrossingTrackWidget::getFibs( QAbstractItemModel* roiModel )
{
    QList<Dataset*> l;
...
void CrossingTrackWidget::start()
{
    ...
    qDebug() << VPtr<Dataset>::asPtr( m_dsl[m_maskSelect->getCurrentIndex()-1] )->properties()->get( Fn::Property::NAME ).toString();
    qDebug() << VPtr<Dataset>::asPtr( m_dsl[m_tensor2Select->getCurrentIndex()-1] )->properties()->get( Fn::Property::NAME ).toString();
    qDebug() << VPtr<Dataset>::asPtr( m_dsl[m_tensor3Select->getCurrentIndex()-1] )->properties()->get( Fn::Property::NAME ).toString();
