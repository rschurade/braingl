...
void CrossingTrackWidget::start()
{
    ...
    qDebug() << VPtr<Dataset>::asPtr( m_dsl[m_maskSelect->getCurrentIndex()-1] )->properties()->get( FNPROP_NAME ).toString();
    qDebug() << VPtr<Dataset>::asPtr( m_dsl[m_tensor2Select->getCurrentIndex()-1] )->properties()->get( FNPROP_NAME ).toString();
    qDebug() << VPtr<Dataset>::asPtr( m_dsl[m_tensor3Select->getCurrentIndex()-1] )->properties()->get( FNPROP_NAME ).toString();
